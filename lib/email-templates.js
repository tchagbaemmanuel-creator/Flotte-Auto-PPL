/**
 * Textes des e-mails Flotte PPL — modifiables ici.
 * Variables d'environnement optionnelles :
 *   MAIL_ORG_NAME  (défaut : Flotte PPL)
 *   MAIL_SIGNATURE (défaut : Service Logistique — Probionix PPL)
 */
...
const SIGNATURE = () => (process.env.MAIL_SIGNATURE || 'Service Logistique — Probionix PPL').trim();
      ...
      <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 2px 12px rgba(139,26,26,.12)">
        <tr>
          <td style="background:linear-gradient(135deg,#8B1A1A,#6B1313);padding:22px 28px">
            <div style="display:block;font-size:11px;color:#C8A84B;letter-spacing:2px;text-transform:uppercase">Probionix PPL</div>
